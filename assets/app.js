// DRAIQ LP runtime sections and interactions
(function(){
  ...
  if(orange){
    ...
    <section class="section impact-section" id="impact"><div class="container">
      <div class="section-heading center impact-heading"><p class="eyebrow">IMPACT SIMULATION</p><h2>数字で見る、<br><span>積算AI導入のインパクト。</span></h2><p>見積作成10時間、月20件、1時間あたり3,000円として算出した導入シミュレーション例です。</p></div>
      <div class="impact-table-wrap"><table class="impact-table"><thead><tr><th>指標</th><th>導入前</th><th>導入後</th><th>効果</th></tr></thead><tbody>
        ...
        <tr><th>年間削減時間</th><td>2,400時間</td><td>240時間</td><td><strong>2,160時間削減</strong></td></tr>
        <tr><th>人件費換算</th><td>60万円</td><td>6万円</td><td><strong>月54万円相当</strong></td></tr>
        <tr><th>年間人件費換算</th><td>720万円</td><td>72万円</td><td><strong>年648万円相当</strong></td></tr>
      </tbody></table></div>
    ...
    <section class="section workflow-section" id="workflow"><div class="container">
      <div class="section-heading center workflow-heading"><p class="eyebrow">BEFORE / AFTER</p><h2>導入前と導入後で、<br><span>積算業務はこう変わります。</span></h2><p>見積作成の時間を減らすだけではありません。積算工程そのものを整理し、人が行う判断とAIが担う処理を分けることで、業務全体の流れを変えていきます。</p></div>
      <div class="workflow-compare">
        <div class="workflow-column before"><div class="workflow-column-head"><span>BEFORE</span><h3>導入前</h3><p>確認・検索・転記・整理まで、人が抱え込みやすい状態。</p></div><ol class="workflow-list">
        ...
        <div class="workflow-column after"><div class="workflow-column-head"><span>AFTER</span><h3>導入後</h3><p>AIが一次処理を支え、人は確認・判断・最終調整に集中。</p></div><ol class="workflow-list">
    ...
    <section class="section implementation-section" id="implementation"><div class="container">
      <div class="section-heading center implementation-heading"><p class="eyebrow">IMPLEMENTATION FLOW</p><h2>いきなりAIを入れません。<br><span>まず、積算業務を整理します。</span></h2><p>現在の積算体制を確認し、AI化できる工程と人が判断すべき工程を切り分けながら、段階的に導入していきます。</p></div>
  ...
  if(contact && !document.getElementById('consultation')){
    ...
    <section class="section consultation-section" id="consultation"><div class="container">
      <div class="consultation-layout">
        <div class="section-heading consultation-heading">
          <p class="eyebrow">WHAT WE REVIEW</p>
          <h2>ご相談では、<br><span>現状の課題を<br>整理します。</span></h2>
          <p>導入を前提に進めることはありません。<br>現在の積算工程や担当人数、見積件数、所要時間、<br>過去データの状態を確認し、<br>AI化できる業務と期待できる効果を整理します。</p>
          <div class="consultation-message"><span>POINT</span><strong>導入ありきではなく、まず現状確認から。</strong></div>
        </div>
  ...
  document.querySelectorAll('.eyebrow').forEach(ey=>{
    if(ey.textContent.trim()==='WHO SHOULD TALK'){
      const h2=ey.parentElement && ey.parentElement.querySelector('h2');
      if(h2) h2.innerHTML='見積業務に課題を感じたら<br>積算体制を見直すサインです';
    }
  });
  const selfcheck=document.getElementById('selfcheck');
  const selfcheckHeading=selfcheck && selfcheck.querySelector('h2');
  if(selfcheckHeading) selfcheckHeading.innerHTML='積算の課題<br>簡易チェック';
})();
...
function updateRiskCheck(){
  ...
  if(count<=1) messageEl.textContent='現時点で該当は少なめです。今後のために、積算工程と担当範囲を整理しておくとAI導入判断がしやすくなります。';
  else if(count<=3) messageEl.textContent='いくつか改善余地があります。どの工程に時間や属人化が集中しているか、一度棚卸しする価値があります。';
  else messageEl.textContent='複数の課題が重なっています。製品選びの前に、現在の積算体制とAI化できる範囲を整理することをおすすめします。';
}
...
document.querySelectorAll('.faq-list details').forEach(detail=>{
  detail.addEventListener('toggle',()=>{
    ...
    document.querySelectorAll('.faq-list details').forEach(other=>{if(other!==detail) other.removeAttribute('open');});
  });
});
